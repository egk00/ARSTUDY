...
public class GameManager2D : MonoBehaviour {
	...
	void RefreshButton()
	{
		for (int i = 0; i < vNbrButton; i++)
		{
			...
			vButtonsList [i].transform.FindChild ("Shape").GetComponent<Image> ().sprite = Resources.Load<Sprite> ("Enemies/" + vUsedForms [i].vForm + "/Border");

			Image vImageInside = vButtonsList [i].transform.FindChild ("Shape").FindChild ("Inside").GetComponent<Image> ();
